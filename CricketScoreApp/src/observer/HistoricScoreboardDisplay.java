...
public class HistoricScoreboardDisplay implements Display {
    @Override
    public void display(Match match) {
        System.out.println("\n\n################### 🏏 SCOREBOARD 🏏 ###################");
        System.out.println("Match ID: " + match.getId() + " | Status: " + match.getState());

        for (int i = 0; i < match.getInnings().size(); i++) {
            Innings innings = match.getInnings().get(i);
            System.out.println("\n--- Innings " + (i + 1) + ": " + innings.getBattingTeam().getName() + " ---");
